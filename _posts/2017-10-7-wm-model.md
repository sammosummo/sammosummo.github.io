...
If the probed item was one of the ones remembered, the subject correctly responds same or different, depending on the type of trial. If the probed item was not remembered, the subject guesses, just like on a lapse trial. The probability that the subject guesses same is assumed to be fixed across both lapse and non-lapse trials (more on this later), and is denoted by \\( g \\).

With the decision process fully defined, we can now simulate the subject’s response on a given trial, provided we know the trial conditions, namely the set size and whether the correct response was same or different, and the values of the parameters \\(z \\), \\( k \\), and \\( g \\). The following Python function generates responses to single trials:
...
It is generally a good idea to do inference on normally distributed variables. However, \\( k \\) cannot be normal, because negative values of \\( k \\) are impossible, and neither can \\( g \\) nor \\( z \\), because they are probabilities and must fall between 0 and 1. Therefore, we let
...
where \\(a \\), \\(b \\), and \\(c \\) index the coefficients. Finally, we also place somewhat vague priors on the standard deviations:

$$
\sigma_{\left(\kappa\right)}\sim{}\textrm{Half-Cauchy}\left(2.5\right)\\
\sigma_{\left(\gamma\right)}\sim{}\textrm{Half-Cauchy}\left(2.5\right)\\
\sigma_{\left(\zeta\right)}\sim{}\textrm{Half-Cauchy}\left(2.5\right)\textrm{.}
$$
...
The model allows the user to specify and measure the effects of any number of independent variables on the three separate elements of the decision process, namely WM capacity ($$k$$), response bias ($$g$$), and lapse rate ($$z$$). The model deals with the fact that these variables are not naturally normally distributed by placing linear models on their transformations. Because the model is hierarchical, it applies regularisation or *shrinkage* to the lower-level stochastics and therefore should deal with outliers and/or noisy data better than traditional (non-hierarchical) analyses.

As mentioned earlier, my model is not the same as Morey’s version. The most important differences are related to the specification of the priors. I specify a separate univariate prior for each coefficient, rather than a single multivariate prior for all coefficients per family of lower-level stochastics. My version will not capture the covariance between coefficients, but personally I think this is overkill. I use Cauchy and Half-Cauchy priors because they have heavy tails; the spread of these distributions (2.5) seemed reasonable to me, but was not thoroughly examined, so anyone using this code for their own research should probably consider modifying this value. Finally, I allow linear models to be placed on the prior means, but not the prior standard deviations. Again, this felt like overkill.

For a real application of the model, please see my recently published paper, which measured the effect of schizophrenia/psychosis on visual working memory[<sup>8</sup>].

[<sup>8</sup>]: https://doi.org/10.1016/j.schres.2017.08.015 "Mathias, S. R., Knowles, E. E. M., Barrett, J., Beetham, T., Leach, O., Buccheri, S., Aberizk, K., Blangero, J., Poldrack, R. A., & Glahn, D. C.. (2017). Deficits in visual working-memory capacity and general cognition in African Americans with psychosis. Schizophrenia Research, in press."

The code
--------

The model is available via this Gist.
