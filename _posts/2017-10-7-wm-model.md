...
With the decision process fully defined, we can now simulate the subject’s response on a given trial, provided we know the trial conditions, namely the value of \\( M \\) and whether the correct response was same or different, and the values of the parameters \\(z \\), \\( k \\), and \\( b \\).
...
To estimate \\(z \\), \\( k \\), and \\( b \\) from data we need to distinguish between *hits*, *misses*, *false alarms*, and *correct rejections* in a similar way to in signal detection theory. We arbitrarily define a hit as a correct response on a different trial, and a false alarm as an incorrect response on a different trial. Hits and false alarms follow the following binomial probability distributions:
