...
$$
F_{i_j} \sim \textrm{Binomial}\left(f_{i_j},S_{i_j}\right)
$$
...
f_{i_j}=\left(1-z_i\right)g_i+z_i\left(1-q_{i_j}\right)g_i
$$

where

$$
q_{i_j}=\min\left(1,\frac{k_i}{M_{i_j}}\right)
$$
...
$$
k_{i}=\max\left(\kappa_i, 0\right)\\
g_{i}=\textrm{logistic}\left(\gamma_i\right)\\
z_{i}=\textrm{logistic}\left(\zeta_i\right)
$$
...
\zeta_i=\mu_{(\zeta)_i}+\delta_{(\zeta)_i}\sigma_{(\zeta)}
$$
...
$$
\vec{\mu_{\left(\kappa\right)}}=\mathbf{X}_{\left(\kappa\right)}\vec{\beta_{\left(\kappa\right)}}\\
\vec{\mu_{\left(\gamma\right)}}=\mathbf{X}_{\left(\gamma\right)}\vec{\beta_{\left(\gamma\right)}}\\
\vec{\mu_{\left(\zeta\right)}}=\mathbf{X}_{\left(\zeta\right)}\vec{\beta_{\left(\zeta\right)}}
$$
...
$$
\beta_{(\kappa)_a}\sim{}\textrm{Cauchy}\left(0, 5\right)\\
\beta_{(\gamma)_b}\sim{}\textrm{Cauchy}\left(0, 5\right)\\
\beta_{(\zeta)_c}\sim{}\textrm{Cauchy}\left(0, 5\right)
$$

where \\(a \\), \\(b \\), and \\(c \\) index the coefficients. The value of 5 seemed sufficiently vague to me, although it was not based on rigorous testing, so anyone using this code should consider modifying it if necessary.

Returning to the definitions of the transformed decision parameters, the $$\delta$$ variables represent the unique subject/condition offsets from the group-level trends. They should have standard normal probability distributions: 

$$
\delta_{(\kappa)_i}\sim{}\textrm{Normal}\left(0, 1\right)\\
\delta_{(\gamma)_i}\sim{}\textrm{Normal}\left(0, 1\right)\\
\delta_{(\zeta)_i}\sim{}\textrm{Normal}\left(0, 1\right)
$$

The $$\sigma$$ variables are scaling factors applied to these offsets, or their standard deviations. They are all stochastic variables, and have the following probability distributions:

$$
\sigma_{(\kappa)}\sim{}\textrm{Half-Cauchy}\left(5\right)\\
\sigma_{(\gamma)}\sim{}\textrm{Half-Cauchy}\left(5\right)\\
\sigma_{(\zeta)}\sim{}\textrm{Half-Cauchy}\left(5\right)
$$

Again, 5 might not be the right choice, but this can be changed very easily.

The code
--------

I have created a GitHub repository for the code [here](https://github.com/sammosummo/wmm.git). To run with an example data set[<sup>8</sup>], download the two scripts somewhere, navigate to that directory and run the following Python snippet:

[<sup>8</sup>]: https://doi.org/10.1073/pnas.0711295105 "Rouder, J. N., Morey, R. D., Cowan, N., Zwilling, C. E., Morey, C. C., & Pratte, M. S. (2008). An assessment of fixed-capacity models of visual working memory. Proceedings of the National Academy of Sciences of the United States of America, 105(16), 5975–5979."
...
As mentioned earlier, my model is not the same as Morey’s version. Besides making use of PyMC3’s advanced sampling methods, I also specify a separate univariate prior for each regression coefficient, rather than a single multivariate prior for all coefficients per family of lower-level stochastic variables, as Morey does. My version will not capture the covariance between coefficients, but personally I think this might be overkill for many applications. Moreover, I allow linear models to be placed on the prior means, but not the prior standard deviations. This is tantamount to assuming that there is equal variance across the levels of a covariate, which might not be accurate in some cases.

The model treats all covariates as fixed effects, in the sense that each covariate has a single coefficient which determines the effect of the covariate on each subject’s data. This makes sense for *between-subjects* covariates, such as age or IQ, where each subject has one value of the covariate. When you have a fully *repeated-measures* design, where each value of the covariate is measured in each subject, it might be better to use an alternative formulation wherein each subject has a separate coefficient per covariate, and subject-level coefficients are drawn from a parent group-level distribution. More on this in a future post.

For a real application of the model, please see my recently published paper[<sup>9</sup>].

[<sup>9</sup>]: https://doi.org/10.1016/j.schres.2017.08.015 "Mathias, S. R., Knowles, E. E. M., Barrett, J., Beetham, T., Leach, O., Buccheri, S., Aberizk, K., Blangero, J., Poldrack, R. A., & Glahn, D. C. (2017). Deficits in visual working-memory capacity and general cognition in African Americans with psychosis. Schizophrenia Research, in press."
