...
$$\begin{align}
\Psi&=dX + Z\\
Z&\sim\textrm{Normal}\left(0, 1\right)
\end{align}$$
...
\textrm{P}\left(\Psi=\psi\mid{}X=1\right)&=\varphi\left(\psi - d\right)\tag{1b}\label{eq:1b}
\end{align}$$
...
$$\begin{align}
\textrm{P}\left(Y=1\mid{}X=0\right)&=\textrm{P}\left(\Psi > k\mid{}X=0\right)\end{align}$$
...
&=\int_{k-d}^{\infty}\varphi\left(\psi\right) \mathrm{d}\psi=\Phi\left(d-k\right)\tag{2b}\label{eq:2b}\end{align}$$
...
$$\begin{align}k=-\Phi^{-1}\left(f\right)\end{align}$$
...
&=\Phi^{-1}\left(h\right)-\Phi^{-1}\left(f\right)\tag{3a}\label{eq:3a}\end{align}$$
