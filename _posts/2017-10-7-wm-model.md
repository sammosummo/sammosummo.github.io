...
\beta_{(\gamma)_b}\sim{}\textrm{Cauchy}\left(0, 2.5\right)\\
\beta_{(\zeta)_c}\sim{}\textrm{Cauchy}\left(0, 2.5\right)\textrm{,}
$$
...
$$
\delta_{(\kappa)_i}\sim{}\textrm{Cauchy}\left(0, 2.5\right)\\
\delta_{(\gamma)_i}\sim{}\textrm{Cauchy}\left(0, 2.5\right)\\
\delta_{(\zeta)_i}\sim{}\textrm{Cauchy}\left(0, 2.5\right)\\
\sigma_{(\kappa)}\sim{}\textrm{Half-Cauchy}\left(2.5\right)\\
\sigma_{(\gamma)}\sim{}\textrm{Half-Cauchy}\left(2.5\right)\\
\sigma_{(\zeta)}\sim{}\textrm{Half-Cauchy}\left(2.5\right)\textrm{.}
$$

Again, 2.5 might not be the right choice, but this can be changed very easily.

The code
--------

I have create a GitHub repository for the code here. Please see the readme for usage instructions. You are more than welcome to use the code for your own research purposes!


A brief discussion
------------------

As mentioned earlier, my model is not the same as Morey’s version. Besides making use of PyMC3’s advanced sampling methods and taking care of funneling, I also specify a separate univariate prior for each regression coefficient, rather than a single multivariate prior for all coefficients per family of lower-level stochastic variables, as Morey does. My version will not capture the covariance between coefficients, but personally I think this is a little overkill for most research. Moreover, I allow linear models to be placed on the prior means, but not the prior standard deviations. This is tantamount to assuming that there is equal variance across conditions in an experiment, which might not be accurate in some cases.
