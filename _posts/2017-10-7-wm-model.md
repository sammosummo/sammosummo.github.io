...
It is generally a good idea to do inference on normally distributed variables. However, \\( k \\) cannot be normal, because negative values of \\( k \\) are impossible, and neither can \\( b \\) nor \\( z \\), because they are probabilities and must fall between 0 and 1. Therefore, we let
