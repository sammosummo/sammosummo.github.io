...
$$
\kappa_{i}=\max\left(k_i, 0\right)\\
\gamma_{i}=\textrm{logistic}\left(g_i\right)\\
...
$$
\kappa_{i}\sim\left(\mu_{(\kappa)_i}, \sigma_{(\mu)_i}\right)\\
$$
...
If we simply want to estimate subjects’ parameter values — assuming that those values are fixed across all trials in the experiment, and are irrespective of any independent variables — we could simply assign normal parent distributions to the lower-level stochastic variables, and hyperpriors to the means and standard deviations of the parents. But in most situations, we are interested in quantifying the effects of one or more independent variables on performance. To do this, we assign linear models to the means of the parent distributions, like so:
