...
- statistics
- cognition
title: Power law assumption of memory precision
---

_Visual short-term memory_ (VSTM) is the name for the perceptual and cognitive processes that allow us to remember the
features of a visual scene for up to several seconds after it has disappeared from view, also called *visual working
memory*. There has been some debate about whether VTSM is better characterized as a store with a fixed number of slots
{{ site.data.refs.Luck2013a.citep }} or as a resource that is distributed flexibly between all items in a scene
{{ site.data.refs.Ma2014a.citep }}. One of the key findings in support of the latter view comes from *delayed-estimation
tasks*.
 
 Experts in the field often use the term _visual working memory_ instead
