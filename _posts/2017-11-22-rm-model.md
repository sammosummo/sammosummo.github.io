...
$$
d\textrm{foil}_{i_j}=\mu^{d_\left{\textrm{foil}\right)}+\eta^{d_\left{\textrm{foil}\right)}\alpha^{d_\left{\textrm{foil}\right)}_i+\zeta^{d_\left{\textrm{foil}\right)}\beta^{d_\left{\textrm{foil}\right)}_j\\
d\textrm{target}_{i_j}=\mu^{d_\left{\textrm{target}\right)}+\eta^{d_\left{\textrm{target}\right)}\alpha^{d_\left{\textrm{target}\right)}_i+\zeta^{d_\left{\textrm{target}\right)}\beta^{d_\left{\textrm{target}\right)}_j
$$

where $$\mu$$ denotes a grand mean, $$\eta$$ denotes a standard deviation of inter-subject variability, $$\alpha$$ denotes a subject offset, $$\eta$$ denotes a standard deviation of inter-item variability, $$\beta$$ denotes a random effect of item, and $$i$$ and $$j$$ index the subject and item, respectively. This parameterisation is intended to avoid funneling when performing MCMC sampling.
