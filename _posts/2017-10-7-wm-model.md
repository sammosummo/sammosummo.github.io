...
where \\(a \\), \\(b \\), and \\(c \\) index the coefficients. The value of 5 seemed reasonable to me, although it was not based on rigorous testing, so anyone using this code should consider modifying it if necessary.
...
$$
\delta_{(\kappa)_i}\sim{}\textrm{Cauchy}\left(0, 5\right)\\
\delta_{(\gamma)_i}\sim{}\textrm{Cauchy}\left(0, 5\right)\\
\delta_{(\zeta)_i}\sim{}\textrm{Cauchy}\left(0, 5\right)\\
