...
[<sup>6</sup>]: https://dx.doi.org/10.3758%2Fs13428-011-0114-8 "Morey, R. D. & Marey, C. C. (2011). WoMMBAT: A user interface for hierarchical Bayesian estimation of working memory capacity. Behavior Research Methods, 43(4), 1044–1065."

I have implemented my own version of this model using PyMC3[<sup>7</sup>]. My version is not exactly the same as Morey’s. In particular, I used different hyperpriors, and my version uses the more modern no-U-turn sampler (NUTS)[<sup>8</sup>], a modern Markov Chain Monte Carlo (MCMC) algorithm, which is vastly more efficient than earlier algorithms (e.g., Metropolis) when sampling from hierarchical models. The code only applies to the Cowan style of change-detection tasks, although it would be easy to modify the code to apply to Pashler-style tasks, or indeed any other variation, provided the decision rule can be specified.